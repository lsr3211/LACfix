 ...
 */
static void BCH_STD_UNUSED compute_syndromes(struct bch_control *bch,
					     uint32_t *ecc,
					     unsigned int *syn)
{
	int i, j, s;
...
/*
 * Public-exponent a^i helper for syndrome and Chien-search positions.
 * Do not use this for indices derived from syndrome values, ELP coefficients,
 * or other secret-dependent field elements.
 */
static uint32x4_t a_pow4_public_neon(struct bch_control *bch,
				     int e0, int e1, int e2, int e3)
{
	uint32_t out[4];

	out[0] = bch->a_pow_tab[modulo(bch, e0)];
	out[1] = bch->a_pow_tab[modulo(bch, e1)];
	out[2] = bch->a_pow_tab[modulo(bch, e2)];
	out[3] = bch->a_pow_tab[modulo(bch, e3)];
					 ...
					 uint32_t bit_mask)
{
	...
	add = a_pow4_public_neon(bch, exp_buf[0], exp_buf[1],
				 exp_buf[2], exp_buf[3]);
...
static unsigned int ct_select_syn(const unsigned int *syn, unsigned int n,
				  unsigned int idx, uint32_t valid_mask)
{
	unsigned int i, out = 0;

	for (i = 0; i < n; i++)
		out = ct_select_u32(valid_mask & ct_mask_eq_u32(i, idx),
				    syn[i], out);

	return out;
}

static void gf_poly_copy_fixed(struct gf_poly *dst,
...
static int BCH_STD_UNUSED compute_error_locator_polynomial(struct bch_control *bch,
					       ...
					       const unsigned int *syn)
{
	...
	for (i = 0; i < t; i++) {
		...
		for (j = 0; j <= t; j++) {
			...
			uint32_t mask_idx_valid =
				(0u - (uint32_t)(idx >= 0)) &
				ct_mask_le_u32(idx_u32, t);
			unsigned int u;

			for (u = 0; u <= t; u++) {
				uint32_t mask_u = ct_mask_eq_u32(u, idx_u32);

				elp->c[u] ^= addv & mask_use &
					     mask_idx_valid & mask_u;
			}
		}
		...
		if (i < t-1) {
			...
			for (j = 1; j <= t; j++) {
				unsigned int sidx = sidx_base - j;
				uint32_t mask_use = ct_mask_le_u32(j, elp->deg) &
					ct_mask_le_u32(j, sidx_base);
				unsigned int synv = ct_select_syn(syn, 2*t, sidx,
								  mask_use);

				d_next ^= ct_gf_mul(bch, elp->c[j], synv) &
					  mask_use;
			}
			d = d_next;
		}
	...
	for (i = 0; i < t; i++) {
		...
		if (i < t-1) {
			...
			for (j = 1; j <= t; j += 4) {
				...
				uint32x4_t prod_vec;
				uint32x4_t mask_vec;
				uint32_t synv_buf[4] = { 0, 0, 0, 0 };
				uint32_t coeff_buf[4] = { 0, 0, 0, 0 };

				for (lane = 0; lane < 4; lane++) {
					unsigned int jj = j + lane;
					uint32_t lane_active = ct_mask_le_u32(jj, t);
					uint32_t syn_in_range = ct_mask_le_u32(jj, sidx_base);
					uint32_t mask_use =
						lane_active &
					...
					if (jj <= sidx_base)
						synv_buf[lane] = syn[sidx_base - jj];
				}
 ...
 * file for details
 */
static int BCH_STD_UNUSED find_poly_roots(struct bch_control *bch, unsigned int k,
					  struct gf_poly *poly,
					  unsigned int *roots)
{
	int cnt;
				...
				struct gf_poly *p, unsigned int *roots)
{
	...
	const unsigned int search_len = bound-start+1;
	const uint32x4_t lane_step = { 0, 1, 2, 3 };
	const uint32x4_t x_step = vdupq_n_u32(a_pow(bch, 4));
	uint32x4_t x_vec = a_pow4_public_neon(bch, start, start+1,
					      start+2, start+3);
	...
	for (i = 0; i < search_len; i += 4) {
		...
		x_vec = ct_gf_mul4(bch, x_vec, x_step);
	}
...
int decode_bch_ct_scalar(struct bch_control *bch, const uint8_t *data,
			 unsigned int len, const uint8_t *recv_ecc,
			 const uint8_t *calc_ecc, const unsigned int *syn,
			 unsigned int *errloc)
{
