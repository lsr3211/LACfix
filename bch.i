     ...
     uint32_t *ecc)
{
 int i;
 const uint32_t *p;
 const int l = ((((bch)->m)*((bch)->t) + 32 - 1) / 32)-1;
        ...
        const uint8_t *src)
{
 ...
 unsigned int i, nwords = ((((bch)->m)*((bch)->t) + 32 - 1) / 32)-1;

 for (i = 0; i < nwords; i++, src += 4)
  dst[i] = (src[0] << 24)|(src[1] << 16)|(src[2] << 8)|src[3];

 memcpy(pad, src, ((((bch)->m)*((bch)->t) + 8 - 1) / 8)-4*nwords);
         ...
         const uint32_t *src)
{
 ...
 memcpy(dst, pad, ((((bch)->m)*((bch)->t) + 8 - 1) / 8)-4*nwords);
}
# 151 "bch.c"
void encode_bch(struct bch_control *bch, const uint8_t *data,
  unsigned int len, uint8_t *ecc)
{
 const unsigned int l = ((((bch)->m)*((bch)->t) + 32 - 1) / 32)-1;
 unsigned int i, mlen;
 unsigned long m;
 uint32_t w, r[l+1];
...
static inline int deg(unsigned int poly)
{
 ...
 return ({ unsigned int __tmp = poly; unsigned int __count = 0; while (__tmp >>= 1) __count++; __count + 1; })-1;
}
      ...
      unsigned int b)
{
 return a ? bch->a_pow_tab[mod_s(bch, bch->a_log_tab[a]+
     ((bch)->n)-bch->a_log_tab[b])] : 0;
}

static inline unsigned int gf_inv(struct bch_control *bch, unsigned int a)
{
 return bch->a_pow_tab[((bch)->n)-bch->a_log_tab[a]];
}
...
{
 return mod_s(bch, ((bch)->n)-bch->a_log_tab[x]);
}
...
{
 memcpy(dst, src, (sizeof(struct gf_poly)+((src->deg)+1)*sizeof(unsigned int)));
}
         ...
         const unsigned int *syn)
{
 ...
 memset(pelp, 0, (sizeof(struct gf_poly)+((2*t)+1)*sizeof(unsigned int)));
 memset(elp, 0, (sizeof(struct gf_poly)+((2*t)+1)*sizeof(unsigned int)));
    ...
    unsigned int *roots)
{
 ...
 if (poly->c[0])
  ...
  roots[n++] = mod_s(bch, ((bch)->n)-bch->a_log_tab[poly->c[0]]+
...
{
 int i, d = a->deg, l = ((bch)->n)-a_log(bch, a->c[a->deg]);
     ...
     struct gf_poly *out)
{
 ...
 out->deg = 0;
 memset(out, 0, (sizeof(struct gf_poly)+((f->deg)+1)*sizeof(unsigned int)));
...
int decode_bch(struct bch_control *bch, const uint8_t *data, unsigned int len,
        ...
        const unsigned int *syn, unsigned int *errloc)
{
 const unsigned int ecc_words = ((((bch)->m)*((bch)->t) + 32 - 1) / 32);
 unsigned int nbits;
 int i, err, nroots;
 uint32_t sum;
...
static void build_mod8_tables(struct bch_control *bch, const uint32_t *g)
{
 ...
 const int l = ((((bch)->m)*((bch)->t) + 32 - 1) / 32);
 const int plen = ((bch->ecc_bits+1 + 32 - 1) / 32);
 const int ecclen = ((bch->ecc_bits + 32 - 1) / 32);
...
static uint32_t *compute_generator_polynomial(struct bch_control *bch)
{
 ...
 g = bch_alloc((sizeof(struct gf_poly)+((m*t)+1)*sizeof(unsigned int)), &err);
 roots = bch_alloc((bch->n+1)*sizeof(*roots), &err);
 genpoly = bch_alloc(((m*t+1 + 32 - 1) / 32)*sizeof(*genpoly), &err);
...
# 1413 "bch.c"
 ...
 bch = memset(malloc(sizeof(*bch)), 0, sizeof(*bch));
 if (bch == 
# 1442 "bch.c" 3 4
           ((void *)0)
# 1442 "bch.c"
 ...
 for (i = 0; i < (sizeof(bch->poly_2t) / sizeof(*(bch->poly_2t))); i++)
  bch->poly_2t[i] = bch_alloc((sizeof(struct gf_poly)+((2*t)+1)*sizeof(unsigned int)), &err);
