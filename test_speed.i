...
int test_poly_mul_speed()
{
 clock_t start,finish;
    double total_time;
 unsigned char a[512],pk[512],seed[32];
 char sk[512];
 int i;

 random_bytes(a,512);
 random_bytes(seed,32);
 gen_psi_fix_ham(sk,512,seed);

    start=clock();
    for(i=0;i<10000;i++)
 {
  poly_mul(a,sk,pk,512);
 }
    finish=clock();
...
int test_pke_speed()
{
 clock_t start,finish;
    double total_time;
 unsigned char pk[544];
 unsigned char sk[512 +544];
 unsigned char k1[32],k2[32],c[712];
 int i;
 unsigned long long mlen=32,clen=712;
...
int test_ake_speed()
{
 clock_t start,finish;
    double total_time;
 unsigned char pk_a[544],pk_b[544],pk[544];
 unsigned char sk[512 +544],sk_a[512 +544],sk_b[512 +544];
 unsigned char k_a[32],k_b[32],c_a[712],c_b[2*712],k1[32];
 int i;
